...
class MirrorModule(private val reactContext: ReactApplicationContext) :
    ...
    companion object {
        ...
        // ─── ROOT FIX for "Invalid MediaProjection data" ─────────────────────
        // Android 10+ invalidates MediaProjection Intent tokens during IPC
        // serialization (putExtra/getParcelableExtra through Binder). The token
        // MUST be held in-process and passed directly to getMediaProjection().
        // Storing here (companion object = same JVM process) guarantees the
        // service reads the exact object the system granted.
        ...
        override fun onActivityResult(
            ...
            if (requestCode == REQUEST_MEDIA_PROJECTION) {
                if (resultCode == Activity.RESULT_OK && data != null) {
                    // Store in memory — never via Intent extras
    ...
    private fun launchCaptureService() {
        ...
        // Only lightweight primitives in the Intent — token stays in companion object
        ...
        // Two separate WritableNativeMap instances — New Arch single-ownership rule
    ...
    private fun startEventPolling() {
        Thread {
            while (true) {
                try {
                    Thread.sleep(500)
                    val events: List<Pair<String, String>> = ScreenCaptureService.pendingEvents.toList()
                    ScreenCaptureService.pendingEvents.clear()
                    events.forEach { pair: Pair<String, String> ->
                        sendStringEvent(pair.first, pair.second)
                    }
                } catch (e: InterruptedException) { break }
                catch (e: Exception) { Log.e(TAG, "Event polling error", e) }
            }
