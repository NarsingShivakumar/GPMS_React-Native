...
type SharingPhase = 'idle' | 'requesting' | 'advertising' | 'connected' | 'error';
...
export default function SharingScreen() {
  ...
  useEffect(() => {
    ...
    return () => { evtSubsRef.current.forEach(s => s?.remove()); };
  }, []);
  ...
  const stopSharing = useCallback(async () => {
    Alert.alert('Stop Sharing?', 'This will disconnect all viewers and stop screen broadcasting.', [
  ...
  const renderIdle = () => (
    <View style={styles.centerBlock}>
      ...
      {!accessibilityEnabled && (
        <TouchableOpacity style={styles.a11yBanner} onPress={() => MirrorModule.openAccessibilitySettings()}>
  ...
  const renderRequesting = () => (
    <View style={styles.centerBlock}>
      ...
      <Text style={styles.sectionSubtitle}>Approve the screen capture request to begin sharing</Text>
    </View>
  );
  ...
  const renderAdvertising = () => (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
      ...
      <TouchableOpacity style={styles.qrToggle} onPress={() => setShowQR(v => !v)} activeOpacity={0.8}>
        <Icon name={showQR ? 'qrcode-minus' : 'qrcode-scan'} size={18} color={colors.accent} />
      ...
      {showQR && connectionInfo?.qrData && (
        <View style={styles.qrContainer}>
          <View style={styles.qrFrame}>
            ...
            <QRCode value={connectionInfo.qrData} size={180} backgroundColor="transparent" color={palette.white} quietZone={12} />
          </View>
  ...
  const renderConnected = () => (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
      <Animated.View style={[styles.connectedBadge, { transform: [{ scale: pulseAnim }] }]}>
        <LinearGradient colors={[`${colors.success}20`, `${colors.success}08`]} style={styles.connectedBadgeInner}>
          <Icon name="access-point-check" size={36} color={colors.success} />
          <Text style={styles.connectedText}>Viewer Connected</Text>
          <Text style={styles.connectedCount}>{clientCount} device{clientCount !== 1 ? 's' : ''}</Text>
        </LinearGradient>
      </Animated.View>
      <View style={styles.codeCard}>
        <Text style={styles.codeLabel}>SHARE CODE</Text>
...
function InfoRow({ icon, label, value, mono = false }: {
  ...
  return (
    <View style={infoStyles.row}>
      ...
      <Text style={[infoStyles.value, mono && infoStyles.mono]} numberOfLines={1}>{value}</Text>
    </View>
  );
}
...
const infoStyles = StyleSheet.create({
  ...
  value: { color: colors.textPrimary, fontSize: typography.sm, fontWeight: typography.medium, maxWidth: '55%' },
...
const styles = StyleSheet.create({
  ...
  iconCircle: { width: 100, height: 100, borderRadius: 50, backgroundColor: `${colors.primary}18`, borderWidth: 1, borderColor: `${colors.primary}30`, alignItems: 'center', justifyContent: 'center', marginBottom: spacing.xl },
  sectionTitle: { fontSize: typography['2xl'], fontWeight: typography.bold, color: colors.textPrimary, textAlign: 'center', marginBottom: spacing.sm },
  sectionSubtitle: { fontSize: typography.base, color: colors.textSecondary, textAlign: 'center', lineHeight: typography.base * 1.6, marginBottom: spacing.xl },
  a11yBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: `${colors.warning}15`, borderWidth: 1, borderColor: `${colors.warning}40`, borderRadius: radii.md, padding: spacing.sm, paddingHorizontal: spacing.base, gap: spacing.sm, marginBottom: spacing.lg, width: '100%' },
  ...
  startBtnGradient: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: spacing.md + 2, gap: spacing.sm },
  startBtnText: { color: palette.white, fontSize: typography.md, fontWeight: typography.bold },
  ...
  statusText: { color: colors.success, fontSize: typography.sm, fontWeight: typography.semibold },
  codeCard: { backgroundColor: colors.bgCard, borderRadius: radii.xl, borderWidth: 1, borderColor: `${colors.primary}40`, alignItems: 'center', padding: spacing.xl, marginBottom: spacing.base, ...shadows.md },
  codeLabel: { fontSize: typography.xs, fontWeight: typography.bold, color: colors.textMuted, letterSpacing: 2, marginBottom: spacing.sm },
  codeValue: { fontSize: typography['4xl'], fontWeight: typography.bold, color: colors.accent, letterSpacing: 10, fontFamily: 'monospace', marginBottom: spacing.xs },
  ...
  qrToggleText: { color: colors.accent, fontSize: typography.sm, fontWeight: typography.medium },
  ...
  connectedBadge: { marginBottom: spacing.xl, borderRadius: radii.xl, overflow: 'hidden', borderWidth: 1, borderColor: `${colors.success}30` },
  connectedBadgeInner: { alignItems: 'center', padding: spacing.xl },
  connectedText: { fontSize: typography.xl, fontWeight: typography.bold, color: colors.success, marginTop: spacing.sm },
  connectedCount: { fontSize: typography.sm, color: `${colors.success}90`, marginTop: spacing.xs },
  stopBtnOutline: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, borderWidth: 1.5, borderColor: `${colors.error}50`, borderRadius: radii.lg, paddingVertical: spacing.md, marginTop: spacing.sm },
  stopBtnText: { color: colors.error, fontSize: typography.base, fontWeight: typography.semibold },
});
